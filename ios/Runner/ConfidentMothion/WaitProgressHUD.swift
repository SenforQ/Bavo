...
fileprivate let app_constantShow_:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]
...
let app_topicNoti_ = 80.0
//: let kProgressHUD_cornerRadius = 14.0
let noti_triggerDetectShow_ = 14.0
//: let kProgressHUD_alpha        = 0.9
let dataRuleUser_ = 0.9
//: let kBackgroundView_alpha     = 0.6
let kAdConst_ = 0.6
//: let kAnimationInterval        = 0.2
let showDegreeMain = 0.2
//: let kTransformScale           = 0.9
let dataAdvertNoti = 0.9

//: open class ProgressHUD: UIView {
open class WaitProgressHUD: UIView {
    //: required public init?(coder: NSCoder) {
    public required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: app_constantShow_.reversed(), encoding: .utf8)!)
    }

    //: static var shared = ProgressHUD()
    static var shared = WaitProgressHUD()
    ...
    class func hidden() {
        //: show(superView: nil)
        extend(superView: nil)
    }

    //: class func show(superView: UIView?) {
    class func extend(superView: UIView?) {
        //: if superView != nil {
        if superView != nil {
            //: DispatchQueue.main.async {
            DispatchQueue.main.async {
                //: ProgressHUD.shared.frame = superView!.bounds
                WaitProgressHUD.shared.frame = superView!.bounds
                //: ProgressHUD.shared.activityIndicator.center = ProgressHUD.shared.center
                WaitProgressHUD.shared.activityIndicator.center = WaitProgressHUD.shared.center
                //: superView!.addSubview(ProgressHUD.shared)
                superView!.addSubview(WaitProgressHUD.shared)
            }
            //: } else {
        } else {
            //: DispatchQueue.main.async {
            DispatchQueue.main.async {
                ...
                WaitProgressHUD.shared.frame = UIScreen.main.bounds
                //: ProgressHUD.shared.activityIndicator.center = ProgressHUD.shared.center
                WaitProgressHUD.shared.activityIndicator.center = WaitProgressHUD.shared.center
                //: AppConfig.getWindow().addSubview(ProgressHUD.shared)
                MainAppConfig.constant().addSubview(WaitProgressHUD.shared)
            }
        }
        //: ProgressHUD.shared.hud_startAnimating()
        WaitProgressHUD.shared.coordinate()
    }

    //: class func dismiss() {
    class func turnStreet() {
        //: ProgressHUD.shared.hud_stopAnimating()
        WaitProgressHUD.shared.computerAnimating()
    }
    ...
    private func computerAnimating() {
        //: DispatchQueue.main.async {
        DispatchQueue.main.async {
            //: UIView.animate(withDuration: kAnimationInterval) {
            UIView.animate(withDuration: showDegreeMain) {
                ...
                WaitProgressHUD.shared.removeFromSuperview()
            }
        }
    }
    ...
    private lazy var activityIndicator: UIActivityIndicatorView = {
        ...
        indicator.bounds = CGRect(x: 0, y: 0, width: app_topicNoti_, height: app_topicNoti_)
        //: indicator.center = self.center
        indicator.center = self.center
        //: indicator.backgroundColor = .black
        indicator.backgroundColor = .black
        //: indicator.layer.cornerRadius = kProgressHUD_cornerRadius
        indicator.layer.cornerRadius = noti_triggerDetectShow_
...
extension WaitProgressHUD {
    //: class func toast(_ str: String?) {
    class func previous(_ str: String?) {
        //: toast(str, showTime: 1)
        trigger(str, showTime: 1)
    }

    //: class func toast(_ str: String?, showTime: CGFloat) {
    class func trigger(_ str: String?, showTime: CGFloat) {
        ...
        MainAppConfig.constant().addSubview(titleLab)
        ...
        titleLab.center = MainAppConfig.constant().center
